...
exchange.check_privilege = function(listname,playername,meta)
...
exchange.update_formspec = function(meta)
	formspec = exchange.formspec.main
	pl_formspec = function (n)
		if meta:get_int(n.."step")==0 then
			formspec = formspec .. exchange.formspec[n].start
		else
			formspec = formspec .. exchange.formspec[n].player(meta:get_string(n))
			if meta:get_int(n.."step") == 1 then
				formspec = formspec .. exchange.formspec[n].accept1
			elseif meta:get_int(n.."step") == 2 then
				formspec = formspec .. exchange.formspec[n].accept2
			end
		end
	end
...
exchange.give_inventory = function(inv,list,playername)
...
exchange.cancel = function(meta)
	exchange.give_inventory(meta:get_inventory(),"pl1",meta:get_string("pl1"))
	exchange.give_inventory(meta:get_inventory(),"pl2",meta:get_string("pl2"))
	meta:set_string("pl1","")
	meta:set_string("pl2","")
...
exchange.exchange = function(meta)
	exchange.give_inventory(meta:get_inventory(),"pl1",meta:get_string("pl2"))
	exchange.give_inventory(meta:get_inventory(),"pl2",meta:get_string("pl1"))
	meta:set_string("pl1","")
	meta:set_string("pl2","")
	meta:set_int("pl1step",0)
	meta:set_int("pl2step",0)
...
minetest.register_node("exchange:chest", {
		...
		pl_receive_fields = function(n)
			...
			if sender:get_player_name() == meta:get_string(n) then
				...
				if meta:get_int(n.."step")==2 and fields[n.."_accept2"] then
					...
					if n == "pl1" and meta:get_int("pl2step") == 3 then exchange.exchange(meta) end
					if n == "pl2" and meta:get_int("pl1step") == 3 then exchange.exchange(meta) end
				end
				if fields[n.."_cancel"] then exchange.cancel(meta) end
			end
		end
	...
	allow_metadata_inventory_move = function(pos, from_list, from_index, to_list, to_index, count, player)
		local meta = minetest.env:get_meta(pos)
		if not exchange.check_privilege(from_list,player:get_player_name(),meta) then return 0 end
		if not exchange.check_privilege(to_list,player:get_player_name(),meta) then return 0 end
		return count
	end,
	allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		local meta = minetest.env:get_meta(pos)
		if not exchange.check_privilege(listname,player:get_player_name(),meta) then return 0 end
